...
impl CoreArith for SoroNum<I256> {
    fn add<const CALC_SCALE: u32, const SCALE_OUT: u32>(
        ...
        let self_scaled = self.value
            .mul(&I256::from_i32(env, 10).pow(CALC_SCALE - self.scale));
        let other_scaled = other.value
            .mul(&I256::from_i32(env, 10).pow(CALC_SCALE - other.scale));
        
        let result = self_scaled.add(&other_scaled);
        let scaled_result = result
            .mul(&I256::from_i32(env, 10).pow(SCALE_OUT))
            .div(&I256::from_i32(env, 10).pow(CALC_SCALE));
        ...
        if return_i256 {
            ...
            match scaled_result.to_i128() {
                Some(value) => Ok(SoroResult::I128(SoroNum {
                    value,
                    scale: SCALE_OUT,
                })),
    ...
    fn div<const CALC_SCALE: u32, const SCALE_OUT: u32>(
        ...
        let self_scaled = self.value
            .mul(&I256::from_i32(env, 10).pow(CALC_SCALE * 2 - self.scale));
        let other_scaled = other.value
            .mul(&I256::from_i32(env, 10).pow(CALC_SCALE - other.scale));
